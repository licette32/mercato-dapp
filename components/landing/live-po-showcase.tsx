...
const EVENTS = [
  { icon: TrendingUp, text: 'InverCap SA funded $48,500 USDC', time: '2m ago' },
  { icon: ShieldCheck, text: 'Milestone 1 released — $24,250 to supplier', time: '18m ago' },
  { icon: CheckCircle2, text: 'Acero del Pacífico confirmed production start', time: '1d ago' },
]
  ...
  return (
    <div className="w-full">
      ...
      >
        ...
        <div className={cn('space-y-4 p-5', isDark ? 'bg-[#0A140D]' : 'bg-card')}>
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: 'Order value', value: '$48,500', sub: 'USDC', accent: false },
              { label: 'Investor APR', value: '12.5%', sub: 'annual', accent: true },
              { label: 'Timeline', value: 'Day 18', sub: 'of 45', accent: false },
            ].map((m) => (
              <div
                key={m.label}
                className={cn(
              ...
              >
                ...
                <p className={cn('text-sm font-bold tabular-nums', m.accent ? 'text-brand-mid dark:text-brand-light' : isDark ? 'text-white' : 'text-foreground')}>
                  {m.value}
                </p>
          ...
          <div>
            ...
            <div className={cn('relative h-1.5 overflow-hidden rounded-full', isDark ? 'bg-white/10' : 'bg-brand-pale')}>
              <div
          ...
          <div className={cn('flex items-center gap-3 rounded-xl border p-3', isDark ? 'border-brand-light/10 bg-white/[0.03]' : 'border-border bg-brand-ultra/80')}>
            ...
            <div className="min-w-0">
              <p className={cn('text-[11px] font-medium', isDark ? 'text-white' : 'text-foreground')}>
                3 participants · Milestone 1 released
              </p>
              <p className="text-[10px] text-muted-foreground">$24,250 paid to supplier</p>
            </div>
          </div>
      ...
      <div className="mt-4 flex flex-wrap items-center justify-center gap-2">
        {[
          { icon: ShieldCheck, text: '127 verified suppliers' },
          { icon: Star, text: '4.8 avg reputation' },
          { icon: TrendingUp, text: '8–15% investor APR' },
        ].map(({ icon: Icon, text }) => (
          <span
            key={text}
